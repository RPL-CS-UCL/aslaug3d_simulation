...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        while collides:
            ...
            x_min = 4.9999
            x_max = 5.0000
            x_coord = 5.0 + self.np_random.uniform(-0.01, 0.01)
            y_min = 3.1499
            y_max = 3.1500
            y_coord = 3.0 + self.np_random.uniform(-0.01, 0.01)
            ...
            for i in range(len(self.actuator_selection)):
                if self.actuator_selection[i]:
                    ...
                    if i == 0:
                        j = self.np_random.uniform(-np.pi*5.0/180.0, np.pi*5.0/180.0)
                    if j == 3:
                        j = self.np_random.uniform(-np.pi*5.0/180.0, np.pi*5.0/180.0)
                    pb.resetJointState(self.robotId, self.joint_mapping[i],
                                       j, 0.0, self.clientId)
    ...
    def spawn_additional_objects(self):
        ...
        sg.add_shelf(pos[0]+halfExtents[0], halfExtents[0]*2, halfExtents[1]*2, 0)
        id = pb.createMultiBody(0, colBoxId, visBoxId, pos_i)
        ids.append(id)

        self.occmap.add_rect([pos[0]+halfExtents[0], pos[1]+halfExtents[1]],
                             halfExtents[0]*2, halfExtents[1]*2)
        pos += np.array((halfExtents[0]*2, 0, 0))
        ...
        # Spawn shelves, row 1
        pos = np.array([4.5,-0.2,0.0])

        halfExtents = [1.0, 0.1, 1.5]
        colBoxId = pb.createCollisionShape(pb.GEOM_BOX,
                                           halfExtents=halfExtents)
        visBoxId = pb.createCollisionShape(pb.GEOM_BOX,
                                           halfExtents=halfExtents)
