...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def setup_action_observation_spaces(self):
        ...
        self.obs_slicing = [self.obs_slicing[i] for i in range(6)] + [self.obs_slicing[5]+self.latent_dim]        

        ob_space = self.observation_space
        ob_space_low =  ob_space.low[:self.obs_slicing[5]].tolist()
        ob_space_low = np.array(ob_space_low + 128*[-1.0])
        ob_space_high =  ob_space.high[:self.obs_slicing[5]].tolist()
        ob_space_high = np.array(ob_space_high + 128*[1.0])
    ...
    def encode_obs (self, obs):
        ...
        scans = torch.Tensor(scans).view(1,1,self.n_scans*2)
        scans = scans#.to("cpu")
        ret, lat = self.LAE(scans)
        lat = lat.detach().numpy()
        obs = np.concatenate((obs[:self.obs_slicing[5]], lat[0]), axis=0)
        return obs
